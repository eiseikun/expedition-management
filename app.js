...
function render(){
  ...
  players.forEach((p,i)=>{
    ...
    tr.innerHTML = `
      <td>${p.name}</td>
      <td>${p.power.toLocaleString('en-US')} </td>
      ...
      <td><input type="checkbox" ${p.expedition?"checked":""} onchange="toggleExpedition(${i})"></td>
      <td>${p.lane}</td>
...
window.editPlayer = function(i){
  ...
  // カオスチェック
  const checkboxes = document.querySelectorAll("#gearChaosContainer input[type=checkbox]");
  checkboxes.forEach(cb => cb.checked = p.gearChaos?.includes(cb.value) || false)
...
window.savePlayer = async function(){
  ...
  document.querySelectorAll("#gearChaosContainer input[type=checkbox]").forEach(cb=>{
    if(cb.checked) gearChaos.push(cb.value)
  })
