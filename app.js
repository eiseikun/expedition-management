...
function runeHTML(name, quality, enchant) {
  ...
  const cls = quality === "ミシック" || quality==="mythic" ? "rune rune-mythic" : "rune rune-legend";
  return `<span class="${cls}">${name}(${enchant})</span>`;
}

function gearText(type, chaosArr) {
  if(!chaosArr || chaosArr.length === 0) return type;
  return `${type}/${chaosArr.join(",")}カオス`;
}
...
window.savePlayer = async function() {
  ...
  if(editIndex === null) {
    const docRef = await addDoc(playersCol, p);
    players.push(p);
...
window.saveTableImage = function() {
  ...
  html2canvas(table, {scale:3}).then(canvas=>{
    ...
    link.download = "players_table.png";
