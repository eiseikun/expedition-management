...
function openModal(player = null) {
  ...
  if (player) {
    ...
    for (let k in player.equipment.chaos) {
      const el = document.getElementById("chaos" + k.charAt(0).toUpperCase() + k.slice(1));
...
playerForm.addEventListener("submit", async (e) => {
  ...
  const data = {
    ...
    artifactRate: Number(document.getElementById("artifactMythic").value) * 0.25 +
                  Number(document.getElementById("artifactLegend").value) * 0.025
  };
  if (playerId) {
