...
document.getElementById("editor").addEventListener('touchmove', e=>{ e.stopPropagation(); }, {passive:false});

function relicBuff(m,l){ return Number((m*0.25 + l*0.025).toFixed(3)); }
function runeHTML(name,q,e){ if(q==="none") return ""; const cls = q==="mythic"?"rune rune-mythic":"rune rune-legend"; return `<span class="${cls}">${name}(${e})</span>`; }
...
window.savePlayer = async function(){
  ...
  document.querySelectorAll("#chaos select").forEach(s=>{
    if(s.value) gearDetail.push({part:s.dataset.part,type:s.value});
  });
...
function render(){
  ...
  [1,2,3,0].forEach(laneNum=>{
    ...
    lanePlayers.forEach(p=>{
      ...
      const displayPower=p.power>=1000000?(p.power/1000000).toFixed(1)+"M":p.power;
